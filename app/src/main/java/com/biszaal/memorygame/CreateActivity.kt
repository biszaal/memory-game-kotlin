...
import com.biszaal.memorygame.utils.*
...
class CreateActivity : AppCompatActivity()
{
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == READ_EXTERNAL_PHOTOS_CODE)
        {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED)
            {
                ...
                Toast.makeText(this, "In order to create a custom game, you need to provice access to your photos", Toast.LENGTH_LONG)
            }
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?)
    {
        ...
        {
            Log.w(TAG, "Did not get back from the lunched activity, user likely canceled flow")
            return
        }
        ...
        db.collection("games").document(customGameName).get().addOnSuccessListener { document ->
            if (document != null && document.data != null)
            {
                AlertDialog.Builder(this)
                    .setTitle("Name taken")
                    .setMessage("A game is already exists with the name '$customGameName'. Please choose another name.")
    ...
    private fun getImageByteArray(photoUri: Uri): ByteArray {
        val originalBitmap: Bitmap = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P)  // if running new version of Android
        {
